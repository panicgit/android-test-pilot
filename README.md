...
android-test-pilot은 **logcat을 1순위 정보 소스**로 사용하여 이 문제를 해결합니다.
...
android-test-pilot:
  logcat 텍스트 → 즉시 판단 → 다음 동작 → logcat → ...
  (텍스트 기반, 빠르고 저렴)
  ↘ 판단 불가 시에만 uiautomator → 스크린샷 fallback
```

logcat 로그는 텍스트이므로 이미지 대비 토큰 소모가 극히 적고, 파싱도 즉시 가능합니다.  
...
Step 2 — 실제 디바이스 테스트 실행
         Tier 1: logcat → Tier 2: uiautomator → Tier 3: screenshot
```
...
| Tier | 도구 | 사용 조건 |
|------|------|-----------|
| Tier 1 | logcat streaming | 가장 먼저 시도. 화면 상태 + API 데이터 실시간 파악 |
| Tier 2 | uiautomator + accessibility tree | Tier 1로 판단 불가 시. resource-id 기반 UI 조작 |
...
```bash
# android-test-pilot의 skills를 테스트 대상 프로젝트에 복사
